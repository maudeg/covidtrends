...
curva.epidemica <- read.table("/Users/maurizio/repoSW/covid/covidtrends-data/data/curva-epidemica/curva_epidemica_Italia_latest.txt")
...
data <- curva.epidemica[sel.date,1] 
...
##lines(curva.epidemica[,2], type='l', lwd=2, col='gray')
#axis(1, at=1:length(R.medio), label=date, las=2)
#axis(2, las=2)
##plot(curva.epidemica[,2], type='l', lwd=2, col='gray', axes=FALSE)

rt <- stima$R$`Mean(R)`
Rtdata<- data.frame(data,rt)
write.csv(Rtdata, file="/tmp/rtdata.csv",row.names = FALSE)